...
BYTE CalcVal(_In_ LPCBYTE buffer, _In_ ULONGLONG size)
{ /// *char
	std::string input((char*) buffer, size);

	//std::ofstream myfile("C:\\Users\\Administrator\\Desktop\\AMSI_GP\\script.txt", std::ios_base::app);
	...
	auto result = client.call("model_calc", input).as<int>();
...
HRESULT SampleAmsiProvider::Scan(_In_ IAmsiStream* stream, _Out_ AMSI_RESULT* result)
{
	...
	if (contentAddress)
	{
		...
		auto result = CalcVal(contentAddress, contentSize);
		TraceLoggingWrite(g_traceLoggingProvider, "calcval",
			TraceLoggingValue(requestNumber),
	...
	else
	{
		...
		for (ULONGLONG position = 0; position < contentSize; position += readSize)
		{
			...
			if (SUCCEEDED(hr))
			{
				int result = CalcVal(chunk, readSize);
